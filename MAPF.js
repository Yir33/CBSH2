...
function move(x , y , state){
    ...
    context1.fillRect((y-1)*width, (x-1)*height , 0.95*width, 0.95*height);
}
...
function solve(j){
    ...
    for(let i = 0 ; i < paths.length ; i ++){
        if(j >= paths[i].length){
            move(paths[i][paths[i].length-3] , paths[i][paths[i].length-2] ,1);
        }else{
            move(paths[i][j] , paths[i][j+1] , 0);
        }
    }
}
function draw(){
